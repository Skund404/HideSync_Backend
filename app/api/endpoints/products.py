...
def list_products(
    *,
    ...
    search_query: Optional[str] = Query(
        None, alias="searchQuery", description="Search by name, SKU, description"
    ),
    status: Optional[List[str]] = Query(
        None, description="Filter by inventory status (e.g., IN_STOCK)"
    ),
    product_type: Optional[List[str]] = Query(
        None, alias="productType", description="Filter by product type (e.g., WALLET)"
    ),
    storage_location: Optional[str] = Query(
        None, alias="storageLocation", description="Filter by storage location"
    ),
    date_from: Optional[str] = Query(
        None,
        alias="dateAddedRange[from]",
        description="Start date for date added (YYYY-MM-DD)",
    ),
    date_to: Optional[str] = Query(
        None,
        alias="dateAddedRange[to]",
        description="End date for date added (YYYY-MM-DD)",
    ),
    min_price: Optional[float] = Query(
        None, alias="priceRange[min]", ge=0, description="Minimum selling price"
    ),
    max_price: Optional[float] = Query(
        None, alias="priceRange[max]", ge=0, description="Maximum selling price"
    ),
    pattern_id: Optional[int] = Query(
        None, alias="patternId", ge=1, description="Filter by pattern ID"
    ),
    project_id: Optional[int] = Query(
        None, alias="projectId", ge=1, description="Filter by project ID"
    ),
) -> ProductList:
    """
    ...
    try:
        filters = ProductFilter(
            ...
            dateAddedRange=(
                {"from": date_from, "to": date_to} if date_from or date_to else None
            ),
        ...
        logger.debug(
            f"Constructed ProductFilter: {filters.model_dump(exclude_none=True)}"
        )
    except (
        Exception
    ) as e:  # Catch potential Pydantic validation errors during filter creation
    ...
    try:
        ...
        items_orm: List[Product] = paginated_data.get(
            "items", []
        ...
        logger.info(
            f"Repository returned {len(items_orm)} products. Total matching: {total}. Preparing page {current_page}/{pages}."
        )
        ...
        for item_orm in items_orm:
            ...
            if not hasattr(item_orm, "inventory") or item_orm.inventory is None:
                logger.warning(
                    f"Inventory data missing for Product ID {item_orm.id}. Setting defaults for response."
                )
            ...
            try:
                response_item = ProductResponse.model_validate(
                    product_data_for_response
                )
                ...
                logger.error(
                    f"Pydantic validation failed for product ID {item_orm.id}: {validation_err}",
                    exc_info=True,
                )
        ...
        return ProductList(
            items=items_response, total=total, page=current_page, size=size, pages=pages
        )
...
def create_product(
    *,
    ...
    provided_sku_for_logging = (
        product_in.sku if product_in.sku else "None (Auto-Generate)"
    )
    logger.info(
        f"Create product request received. User: {user_id}. Provided SKU: '{provided_sku_for_logging}'"
    )
    try:
        ...
        logger.info(
            f"Product and initial inventory created successfully. Product ID: {created_product_orm.id}, SKU: {created_product_orm.sku}"
        )
        ...
        if (
            not hasattr(created_product_orm, "inventory")
            or created_product_orm.inventory is None
        ):
            # This indicates a problem with the refresh or relationship setup
            logger.error(
                f"CRITICAL: Inventory relationship not loaded on product {created_product_orm.id} after creation and refresh."
            )
            ...
            logger.debug(
                f"Inventory details loaded for response: Qty={inventory_quantity}, Status={inventory_status}, Loc={inventory_location}"
            )
        ...
        try:
            ...
            logger.error(
                f"Pydantic validation failed for newly created product ID {created_product_orm.id}: {validation_err}",
                exc_info=True,
            )
        ...
        logger.warning(
            f"Business rule violation creating product (Provided SKU: '{provided_sku_for_logging}'): {error_message}"
        )
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=error_message)
    except HideSyncException as e:
        logger.error(
            f"Internal error creating product (Provided SKU: '{provided_sku_for_logging}'): {e}",
            exc_info=True,
        )
        raise HTTPException(
        ...
        logger.error(
            f"Unexpected error creating product (Provided SKU: '{provided_sku_for_logging}'): {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def get_product(
    *,
    ...
    logger.info(
        f"Get product request received for ID: {product_id} by user: {getattr(current_user, 'id', 'N/A')}"
    )
    try:
        product_orm = product_service.get_product_by_id(
            product_id=product_id, load_inventory=True
        )
        if not product_orm:
        ...
        logger.error(
            f"Unexpected error retrieving product ID {product_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def update_product(
    *,
    ...
    try:
        ...
        logger.info(
            f"Product ID {product_id} update committed successfully by service."
        )
        ...
        refreshed_product_orm = product_service.get_product_by_id(
            product_id=product_id, load_inventory=True
        )
        if not refreshed_product_orm:
            # This would be highly unusual if the update succeeded
            logger.error(
                f"CRITICAL: Could not retrieve product {product_id} immediately after successful update."
            )
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        try:
            ...
            logger.info(
                f"Successfully prepared response for updated product {product_id}."
            )
            return validated_response
        except Exception as validation_err:
            logger.error(
            ...
            try:
                ...
                manual_data = {
                    ...
                    "profit_margin": getattr(
                        refreshed_product_orm, "profit_margin", None
                    ),
                    ...
                    "sales_velocity": getattr(
                        refreshed_product_orm, "sales_velocity", None
                    ),
                }

                # Add inventory-related fields with careful normalization
                if (
                    hasattr(refreshed_product_orm, "inventory")
                    and refreshed_product_orm.inventory
                ):
                    ...
                    manual_data["storage_location"] = (
                        refreshed_product_orm.inventory.storage_location
                    )
                else:
                ...
                logger.info(
                    f"Successfully prepared response using manual approach for product {product_id}."
                )
                return validated_response
            except Exception as manual_err:
                logger.error(
                    f"Manual approach also failed for product ID {refreshed_product_orm.id}: {manual_err}",
                    exc_info=True,
                )
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    # --- Exception Handling ---
        ...
        error_message = str(getattr(e, "details", e))
        logger.warning(
            f"Business rule violation updating product ID {product_id}: {error_message}"
        )
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=error_message)
    except HideSyncException as e:  # Catch internal service errors
        logger.error(
            f"Internal service error updating product ID {product_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e)
        ...
        logger.error(
            f"Unexpected error updating product ID {product_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def delete_product(
    *,
    ...
    user_id = getattr(current_user, "id", None)
    logger.info(
        f"Delete product request received for ID: {product_id} by user: {user_id}"
    )
    try:
        ...
        if not deleted:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Product with ID {product_id} not found or deletion failed.",
            )
        ...
        error_message = str(getattr(e, "details", e))
        logger.warning(
            f"Business rule violation deleting product ID {product_id}: {error_message}"
        )
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail=error_message)
    except HideSyncException as e:
        logger.error(
            f"Internal error deleting product ID {product_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(e)
        )
        ...
        logger.error(
            f"Unexpected error deleting product ID {product_id}: {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def get_product_by_sku(
    *,
    ...
    """
    logger.info(
        f"Get product by SKU request received for SKU: '{sku}' by user: {getattr(current_user, 'id', 'N/A')}"
    )
    try:
        ...
        product_orm = product_service.get_product_by_sku(
            sku=sku
        )  # Service returns Optional[Product]
        if not product_orm:
        ...
        logger.error(
            f"Unexpected error retrieving product by SKU '{sku}': {e}", exc_info=True
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
def trigger_cost_calculation(
    *,
    product_id: int = Path(
        ..., ge=1, description="The ID of the product to calculate costs for"
    ),
    product_service: ProductService = Depends(get_product_service),
    current_user: Any = Depends(get_current_active_user),
) -> ProductResponse:
    ...
    user_id = getattr(current_user, "id", None)
    logger.info(
        f"Trigger cost calculation request received for Product ID: {product_id} by user: {user_id}"
    )
    try:
        ...
        cost_breakdown_result = product_service.calculate_cost_breakdown(
        ...
        updated_product_orm = product_service.get_product_by_id(
            product_id=product_id, load_inventory=True
        )
        if not updated_product_orm:
            # Should not happen if calculate_cost_breakdown succeeded, but defensive check
            logger.error(
                f"Product {product_id} not found after successful cost calculation."
            )
            raise EntityNotFoundException("Product", product_id)

        logger.info(
            f"Cost calculation successful for product ID {product_id}. New total cost: {updated_product_orm.total_cost}"
        )
        ...
        error_message = str(getattr(e, "details", e))
        logger.warning(
            f"Entity not found during cost calculation for product ID {product_id}: {error_message}"
        )
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
        ...
        error_message = str(getattr(e, "details", e))
        logger.error(
            f"Error during cost calculation for product ID {product_id}: {error_message}",
            exc_info=True,
        )
        ...
        raise HTTPException(
            status_code=status_code, detail=f"Cost calculation failed: {error_message}"
        )
    except Exception as e:
        logger.error(
            f"Unexpected error during cost calculation for product ID {product_id}: {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
