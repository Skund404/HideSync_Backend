...
def update_purchase(
    ...
    try:
        purchase = purchase_service.update_purchase(
            purchase_id,
            purchase_data.dict(exclude_unset=True)
        )
...
def update_purchase_item(
    ...
    try:
        ...
        # Update the item
        item = purchase_service.update_purchase_item(item_id, item_data.dict(exclude_unset=True))
...
def create_purchase_plan(
        ...
        include_pending: bool = Query(True, description="Include pending purchases in calculations"),
        db: Session = Depends(get_db),
        current_user=Depends(get_current_active_user)
) -> Any:
    """
