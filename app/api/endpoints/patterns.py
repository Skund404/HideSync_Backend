...
def update_pattern(
        ...
        increment_version: bool = Query(False, description="Whether to increment pattern version"),
        db: Session = Depends(get_db),
        current_user=Depends(get_current_active_user)
) -> Any:
    """
...
@router.post("/{pattern_id}/clone", response_model=PatternResponse, status_code=status.HTTP_201_CREATED)
def clone_pattern(
    ...
    try:
        ...
        cloned_pattern = pattern_service.clone_pattern(pattern_id, new_name, custom_dict)
