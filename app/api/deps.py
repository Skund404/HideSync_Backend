...
from app.core.exceptions import EntityNotFoundException # Import if needed by services directly
...
from app.repositories.pattern_repository import PatternRepository, ProjectTemplateRepository # PatternService needs both
...
def get_current_user(
    ...
    try:
        ...
        logger.error(f"Token validation failed: {e}", exc_info=False) # Reduce noise in logs unless debugging tokens
        raise credentials_exception from e

    try:
        # Use UserRepository directly or instantiate UserService
        # Using UserService is generally cleaner
        user_repo = UserRepository(session=db) # Or get from a factory/registry if available
...
def get_pattern_service(db: Session = Depends(get_db)) -> PatternService:
    ...
    template_repo = ProjectTemplateRepository(session=db) # PatternService needs this too
