...
class SupplierBase(BaseModel):
    ...
    """
    name: str = Field(..., description="Name of the supplier", min_length=1, max_length=100)
    category: Optional[str] = Field(None, description="Primary category (LEATHER, HARDWARE, SUPPLIES, MIXED)")
    ...
    status: Optional[SupplierStatus] = Field(SupplierStatus.ACTIVE, description="Current status of the supplier")
    notes: Optional[str] = Field(None, description="Additional notes about the supplier")
    material_categories: Optional[List[str]] = Field(None, description="Categories of materials supplied")
    logo: Optional[str] = Field(None, description="URL or path to supplier logo")
    payment_terms: Optional[str] = Field(None, description="Payment terms offered by the supplier")
    min_order_amount: Optional[str] = Field(None, description="Minimum order amount if any")
...
class SupplierUpdate(BaseModel):
    ...
    """
    name: Optional[str] = Field(None, description="Name of the supplier", min_length=1, max_length=100)
    category: Optional[str] = Field(None, description="Primary category (LEATHER, HARDWARE, SUPPLIES, MIXED)")
    contact_name: Optional[str] = Field(None, description="Name of primary contact")
    email: Optional[EmailStr] = Field(None, description="Email address of the supplier")
    phone: Optional[str] = Field(None, description="Phone number of the supplier")
    ...
    website: Optional[str] = Field(None, description="Website URL")
    rating: Optional[int] = Field(None, description="Rating from 1-5", ge=1, le=5)
    status: Optional[SupplierStatus] = Field(None, description="Current status of the supplier")
    notes: Optional[str] = Field(None, description="Additional notes about the supplier")
    material_categories: Optional[List[str]] = Field(None, description="Categories of materials supplied")
    logo: Optional[str] = Field(None, description="URL or path to supplier logo")
    payment_terms: Optional[str] = Field(None, description="Payment terms offered by the supplier")
...
class SupplierRatingBase(BaseModel):
    ...
    category: Optional[str] = Field(None, description="Category being rated (quality, delivery, etc.)")
...
class SupplierHistoryBase(BaseModel):
    ...
    event_type: str = Field(..., description="Type of event (ORDER, STATUS_CHANGE, etc.)")
...
class SupplierResponse(SupplierInDB):
    ...
    """
    average_rating: Optional[float] = Field(None, description="Average rating across all categories")
    total_orders: Optional[int] = Field(None, description="Total number of orders placed with this supplier")
    total_spent: Optional[float] = Field(None, description="Total amount spent with this supplier")
    reliability_score: Optional[float] = Field(None, description="Calculated reliability score")
...
class SupplierDetailResponse(SupplierResponse):
    ...
    """
    ratings: List[SupplierRatingInDB] = Field([], description="Recent ratings for this supplier")
    history: List[SupplierHistoryInDB] = Field([], description="Recent history entries for this supplier")
    available_materials: Optional[List[Dict[str, Any]]] = Field(None,
