...
class InventoryBase(BaseModel):
    ...
    item_type: str = Field(
        ...,
        description="Type of item ('product', 'material', 'tool').",
        examples=["product"],
    )
    item_id: int = Field(
        ..., description="ID of the related Product, Material, or Tool.", examples=[101]
    )
    quantity: float = Field(
        ..., description="Current quantity on hand.", ge=0.0, examples=[25.5]
    )
    status: InventoryStatus = Field(
        ...,
        description="Current inventory status (e.g., IN_STOCK).",
        examples=["IN_STOCK"],
    )
    storage_location: Optional[str] = Field(
        None, description="Identifier for the storage location.", examples=["Shelf A1"]
    )
...
class InventoryTransactionBase(BaseModel):
    ...
    quantity_change: float = Field(
        ...,
        description="Quantity change (+ for increase, - for decrease). Cannot be zero.",
        examples=[-5.0],
    )
    transaction_type: TransactionType = Field(
        ...,
        description="Type of transaction (e.g., SALE, PURCHASE, ADJUSTMENT).",
        examples=["PROJECT_USAGE"],
    )
    adjustment_type: Optional[InventoryAdjustmentType] = Field(
        None,
        description="Specific type if transaction_type is ADJUSTMENT.",
        examples=["DAMAGE"],
    )
    reference_id: Optional[str] = Field(
        None,
        description="Optional related record ID (e.g., sale ID, project ID, purchase ID, count ID).",
        examples=["SALE_123", "PROJ_45"],
    )
    reference_type: Optional[str] = Field(
        None,
        description="Type of the related record ('sale', 'project', 'purchase', 'count').",
        examples=["sale"],
    )
    from_location: Optional[str] = Field(
        None,
        description="Source location identifier (for transfers).",
        examples=["Warehouse"],
    )
    to_location: Optional[str] = Field(
        None,
        description="Destination location identifier (for transfers or adjustments affecting location).",
        examples=["Workshop"],
    )
    notes: Optional[str] = Field(
        None,
        description="Additional notes about the transaction.",
        examples=["Used for Project XYZ"],
    )
    performed_by: Optional[str] = Field(
        None,
        description="Identifier (e.g., user ID or name) of who performed the action.",
        examples=["user_10"],
    )
...
class InventoryUpdate(BaseModel):
    ...
    storage_location: Optional[str] = Field(
        None, description="New storage location identifier."
    )
...
class InventoryResponse(InventoryInDB):
    ...
    item_name: Optional[str] = Field(
        None,
        description="Name of the related Product, Material, or Tool.",
        examples=["Leather Wallet V1", "Rawhide Lace Spool"],
    )
...
class InventoryTransactionResponse(InventoryTransactionInDB):
    ...
    item_name: Optional[str] = Field(
        None,
        description="Name of the related Product, Material, or Tool.",
        examples=["Leather Wallet V1"],
    )
    transaction_type_display: Optional[str] = Field(
        None,
        description="Display name for transaction type.",
        examples=["Sale Adjustment"],
    )
    adjustment_type_display: Optional[str] = Field(
        None,
        description="Display name for adjustment type.",
        examples=["Damaged Goods"],
    )
...
class InventoryListResponse(BaseModel):  # Renamed for clarity
    ...
    total: int = Field(
        ..., description="Total number of inventory records matching query."
    )
...
class InventoryAdjustmentRequest(BaseModel):
    ...
    reference_id: Optional[str] = Field(
        None, description="Optional related reference ID."
    )
    reference_type: Optional[str] = Field(
        None, description="Optional related reference type."
    )
    # Location might be updated implicitly based on where adjustment happens or explicitly
    location: Optional[str] = Field(
        None, description="Optional location where adjustment occurs / new location."
    )
...
class InventorySearchParams(BaseModel):
    ...
    location: Optional[str] = Field(
        None, description="Filter by storage location identifier."
    )
    item_type: Optional[str] = Field(
        None, description="Filter by item type ('product', 'material', 'tool')."
    )
    search: Optional[str] = Field(
        None,
        description="Search term (applies to item name/SKU - requires JOIN in repo).",
    )
...
class InventorySummaryResponse(BaseModel):  # Renamed for clarity
    ...
    total_products: int = Field(
        ..., description="Total number of distinct tracked products."
    )
    in_stock: int = Field(
        ..., description="Number of distinct items currently in stock."
    )
    low_stock: int = Field(
        ..., description="Number of distinct items currently in low stock status."
    )
    out_of_stock: int = Field(
        ..., description="Number of distinct items currently out of stock."
    )
    total_value: float = Field(
        ..., description="Total estimated value of all tracked inventory items."
    )
    average_margin: Optional[float] = Field(
        None,
        description="Average profit margin across all tracked products (if calculable).",
    )
    needs_reorder: int = Field(
        ..., description="Number of distinct items at or below their reorder point."
    )
...
class StockLevelReport(BaseModel):  # Kept name as it's specific report type
    ...
    filters_applied: Dict[str, Any] = Field(
        ..., description="Filters used for this report"
    )
    total_value: float = Field(
        ..., description="Total estimated value of reported inventory"
    )
    category_breakdown: Dict[str, Any] = Field(
        ...,
        description="Breakdown by specified category (e.g., type, location, status)",
    )
    low_stock_items: List[InventoryResponse] = Field(
        ..., description="Items below their reorder point"
    )  # Use InventoryResponse
    out_of_stock_items: List[InventoryResponse] = Field(
        ..., description="Items with zero or negative quantity"
    )  # Use InventoryResponse
    # Add more detailed breakdowns as needed
    items_by_location: Optional[Dict[str, int]] = Field(
        None, description="Count of distinct items per location"
    )
    reorder_recommendations: Optional[List[Dict[str, Any]]] = Field(
        None, description="Suggested items and quantities to reorder"
    )
...
):  # Alias for response_model in GET /inventory/transactions/{id} ?
    pass


class InventoryAdjustment(
    InventoryAdjustmentRequest
):  # Alias for request body in POST /inventory/adjust
    pass
