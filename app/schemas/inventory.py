...
class InventoryUpdate(BaseModel):
    ...
    status: Optional[InventoryStatus] = Field(None, description="Current inventory status")
    storage_location: Optional[str] = Field(None, description="Storage location identifier")
...
class InventoryResponse(InventoryInDB):
    ...
    value: Optional[float] = Field(None, description="Total value of the inventory item")
    is_low_stock: Optional[bool] = Field(None, description="Whether the item is in low stock status")
    reorder_point: Optional[float] = Field(None, description="Quantity threshold for reordering")
    days_of_supply: Optional[int] = Field(None, description="Estimated days until stock depletion")
...
class InventoryList(BaseModel):
    ...
    total: int = Field(..., description="Total number of inventory records matching the query")
...
class InventoryTransactionBase(BaseModel):
    ...
    adjustment_type: Optional[InventoryAdjustmentType] = Field(
        None,
        description="Type of adjustment if transaction_type is ADJUSTMENT"
    )
    reference_id: Optional[str] = Field(None, description="Related record ID (e.g., sale, purchase)")
    reference_type: Optional[str] = Field(None, description="Type of the related record")
    from_location: Optional[str] = Field(None, description="Source location for transfers")
    to_location: Optional[str] = Field(None, description="Destination location for transfers")
...
class InventoryTransactionResponse(InventoryTransactionInDB):
    ...
    """
    item_name: Optional[str] = Field(None, description="Name of the inventoried item")
    from_location_name: Optional[str] = Field(None, description="Name of the source location")
    to_location_name: Optional[str] = Field(None, description="Name of the destination location")
    reference_name: Optional[str] = Field(None, description="Name of the referenced entity")
...
class InventorySummary(BaseModel):
    ...
    """
    total_items: int = Field(..., description="Total number of unique items in inventory")
    ...
    inventory_value_by_type: Dict[str, float] = Field(..., description="Value of inventory by type")
...
class InventoryAdjustmentRequest(BaseModel):
    ...
    item_type: str = Field(..., description="Type of item (material, product, tool)")
    item_id: int = Field(..., description="ID of the item")
    quantity: float = Field(..., description="Adjustment quantity (positive for increase, negative for decrease)")
...
class InventoryCountRequest(BaseModel):
    ...
    """
    items: List[Dict[str, Any]] = Field(..., description="List of items and their counted quantities")
