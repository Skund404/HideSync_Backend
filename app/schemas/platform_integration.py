...
class SyncEvent(BaseModel):
    ...
    platform_integration_id: str = Field(..., description="ID of the platform integration")
    created_at: datetime = Field(..., description="Timestamp when the event was created")
    event_type: str = Field(..., description="Type of event (order_import, inventory_update, etc.)")
...
class SyncSettings(BaseModel):
    ...
    """
    auto_sync_enabled: bool = Field(False, description="Whether automatic synchronization is enabled")
    sync_interval_minutes: Optional[int] = Field(None, description="Synchronization interval in minutes", gt=0)
    ...
    export_inventory_changes: bool = Field(True, description="Whether to export inventory changes")
    mark_shipped_orders: bool = Field(True, description="Whether to mark shipped orders as such in the platform")
...
class PlatformIntegrationBase(BaseModel):
    ...
    """
    platform: str = Field(..., description="Platform name (shopify, etsy, amazon, etc.)")
...
class PlatformIntegrationCreate(PlatformIntegrationBase):
    ...
    api_secret: Optional[SecretStr] = Field(None, description="API secret for authentication")
    ...
    def validate_platform(cls, v):
        ...
        allowed_platforms = {"shopify", "etsy", "amazon", "ebay", "woocommerce", "bigcommerce", "squarespace", "custom"}
...
class PlatformIntegrationUpdate(BaseModel):
    ...
    active: Optional[bool] = Field(None, description="Whether the integration is active")
    api_key: Optional[str] = Field(None, description="API key for authentication")
    api_secret: Optional[SecretStr] = Field(None, description="API secret for authentication")
    access_token: Optional[str] = Field(None, description="Access token if available")
    refresh_token: Optional[str] = Field(None, description="Refresh token if available")
...
class SyncResult(BaseModel):
    ...
    details: Optional[Dict[str, Any]] = Field(None, description="Additional synchronization details")
...
class ConnectPlatformRequest(BaseModel):
    ...
    """
    auth_code: Optional[str] = Field(None, description="Authorization code (if using OAuth)")
    shop_url: Optional[str] = Field(None, description="Shop URL (for certain platforms)")
    credentials: Optional[Dict[str, str]] = Field(None, description="Alternative credentials for authentication")
...
class ConnectPlatformResponse(BaseModel):
    ...
    message: str = Field(..., description="Result message")
    integration_id: Optional[str] = Field(None, description="ID of the connected integration")
    connected_at: datetime = Field(..., description="Timestamp of the connection")
    expires_at: Optional[datetime] = Field(None, description="Expiration timestamp for the connection")
    additional_info: Optional[Dict[str, Any]] = Field(None, description="Additional connection information")