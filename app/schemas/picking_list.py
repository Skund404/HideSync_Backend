...
class PickingListItemBase(BaseModel):
    ...
    quantity_ordered: int = Field(..., gt=0, description="Quantity needed, must be greater than 0")
    ...
    def validate_item_reference(cls, v, values):
        ...
        if 'material_id' in values and not values['material_id'] and 'component_id' in values and not values['component_id']:
