...
class MaterialBase(BaseModel):
    ...
    """
    name: str = Field(..., description="Name of the material", min_length=1, max_length=100)
    status: Optional[MaterialStatus] = Field(MaterialStatus.IN_STOCK, description="Current inventory status")
    quantity: float = Field(..., description="Available quantity", ge=0)
    unit: MeasurementUnit = Field(..., description="Unit of measurement")
    quality: Optional[MaterialQualityGrade] = Field(None, description="Quality grade of the material")
    ...
    supplier_sku: Optional[str] = Field(None, description="Supplier's SKU for this material")
    cost: Optional[float] = Field(None, description="Cost per unit", ge=0)
    price: Optional[float] = Field(None, description="Selling price per unit if applicable", ge=0)
    storage_location: Optional[str] = Field(None, description="Location where the material is stored")
    notes: Optional[str] = Field(None, description="Additional notes")
    thumbnail: Optional[str] = Field(None, description="URL or path to thumbnail image")
    material_type: MaterialType = Field(..., description="Type of material (LEATHER, HARDWARE, SUPPLIES)")
...
class LeatherMaterialBase(MaterialBase):
    ...
    is_full_hide: Optional[bool] = Field(False, description="Whether this is a full hide")
    color: Optional[str] = Field(None, description="Color of the leather")
    finish: Optional[LeatherFinish] = Field(None, description="Finish applied to the leather")
...
class SuppliesMaterialBase(MaterialBase):
    ...
    material_composition: Optional[str] = Field(None, description="What the supply is made of")
    volume: Optional[float] = Field(None, description="Volume for liquid supplies", gt=0)
    length: Optional[float] = Field(None, description="Length for linear supplies", gt=0)
    drying_time: Optional[str] = Field(None, description="Drying time for adhesives/finishes")
    application_method: Optional[str] = Field(None, description="How to apply the material")
...
class MaterialUpdate(BaseModel):
    ...
    """
    name: Optional[str] = Field(None, description="Name of the material", min_length=1, max_length=100)
    status: Optional[MaterialStatus] = Field(None, description="Current inventory status")
...
class LeatherMaterialUpdate(MaterialUpdate):
    ...
    thickness: Optional[float] = Field(None, description="Thickness in mm", gt=0)
    area: Optional[float] = Field(None, description="Area in square units", gt=0)
    is_full_hide: Optional[bool] = Field(None, description="Whether this is a full hide")
    color: Optional[str] = Field(None, description="Color of the leather")
    finish: Optional[LeatherFinish] = Field(None, description="Finish applied to the leather")
    grade: Optional[str] = Field(None, description="Quality grade specific to leather")
...
class MaterialInDB(MaterialBase):
    ...
    created_at: datetime = Field(..., description="Timestamp when the material was created")
...
class MaterialResponse(MaterialInDB):
    ...
    """
    inventory_value: Optional[float] = Field(None, description="Total value of inventory (quantity × cost)")
