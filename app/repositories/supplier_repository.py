...
class SupplierRepository(BaseRepository[Supplier]):
    ...
    def list(self, **filters) -> List[Supplier]:
        ...
        try:
            ...
            try:
                ...
                if not use_sqlcipher:
                    logger.info(
                        "SQLCipher not enabled, using standard SQLAlchemy approach for list."
                    )
                ...
                logger.debug(
                    f"Configuring SQLCipher with key format: {key_pragma_value}"
                )
                ...
                if not cursor.fetchone():
                    logger.error(
                        "Table 'suppliers' not found in database via SQLCipher."
                    )
                    conn.close()
                    raise DatabaseException(
                        message="Table 'suppliers' not found.", entity_type="Supplier"
                    )
                ...
                logger.info(
                    f"Executing Supplier List Query: {final_sql} with params: {params}"
                )
                ...
                for row_index, row in enumerate(rows):
                    ...
                    for i, column_name in enumerate(column_names):
                        ...
                        if hasattr(supplier, column_name):
                            try:
                                ...
                                if column_name == "material_categories" and isinstance(
                                    value, str
                                ):
                                    try:
                                        deserialized_value = json.loads(value)
                                        setattr(
                                            supplier, column_name, deserialized_value
                                        )
                                        logger.debug(
                                            f"Row {row_index}: Successfully deserialized material_categories for supplier ID {row_data.get('id', 'N/A')}"
                                        )
                                    except json.JSONDecodeError:
                                        logger.warning(
                                            f"Row {row_index}: Failed to deserialize material_categories '{value}' for supplier ID {row_data.get('id', 'N/A')}. Setting as empty list."
                                        )
                                        setattr(supplier, column_name, [])
                                # Handle potential Enum conversion for status
                                elif column_name == "status" and value is not None:
                                    try:
                                        enum_value = SupplierStatus(
                                            value.lower()
                                        )  # Ensure lowercase matches enum
                                        setattr(supplier, column_name, enum_value)
                                    except ValueError:
                                        logger.warning(
                                            f"Row {row_index}: Invalid status value '{value}' from DB for supplier ID {row_data.get('id', 'N/A')}. Setting attribute as raw value."
                                        )
                                        setattr(
                                            supplier, column_name, value
                                        )  # Set raw value if enum fails
                                else:
                                    setattr(supplier, column_name, value)
                            except Exception as set_err:
                                logger.error(
                                    f"Row {row_index}: Error setting attribute '{column_name}' with value '{value}' (type: {type(value).__name__}) for supplier ID {row_data.get('id', 'N/A')}: {set_err}"
                                )
                        else:
                            logger.warning(
                                f"Row {row_index}: Column '{column_name}' from DB not found in Supplier model. Skipping."
                            )
                    ...
                    if row_index < 5:  # Log details for first few rows
                        ...
                        logger.debug(
                            f"Converted Entity {row_index} (__dict__): {supplier.__dict__}"
                        )
                ...
                if entities and logger.isEnabledFor(logging.DEBUG):
                    ...
                    log_dict = {
                        k: v
                        for k, v in sample_entity.__dict__.items()
                        if not k.startswith("_")
                    }  # Avoid internal SQLAlchemy attrs
                    logger.debug(
                        f"Sample populated entity (ID: {getattr(sample_entity, 'id', 'N/A')}): {log_dict}"
                    )

                conn.close()
                logger.info(
                    f"Successfully retrieved and processed {len(entities)} suppliers via SQLCipher."
                )

                # Decrypt sensitive fields if necessary (assuming _decrypt_sensitive_fields handles None)
                decrypted_entities = [
                    self._decrypt_sensitive_fields(entity) for entity in entities
                ]
            ...
            # --- SQLAlchemy Fallback Path ---
                ...
                logger.error(
                    f"SQLCipher import error: {e}. Falling back to SQLAlchemy approach for list."
                )
                return self._list_with_sqlalchemy(skip, limit, filters)
        ...
        # --- General Exception Handling ---
            ...
            logger.exception(
                f"Unexpected error listing suppliers with filters {filters}: {e}"
            )  # Use logger.exception to include traceback
            raise DatabaseException(
                message=f"Error retrieving suppliers: {str(e)}", entity_type="Supplier"
            )
    ...
    def _list_with_sqlalchemy(
        self, skip: int, limit: int, filters: Dict[str, Any]
    ) -> List[Supplier]:
        """SQLAlchemy fallback implementation for listing suppliers."""
        logger.info(
            f"Using SQLAlchemy ORM to list suppliers. Skip: {skip}, Limit: {limit}, Filters: {filters}"
        )
        try:
            ...
            if "name" in filters and filters["name"]:
                logger.debug(
                    f"Applying SQLAlchemy filter: name LIKE '%{filters['name']}%'"
                )
                query = query.filter(
            ...
            if "status" in filters and filters["status"]:
                try:
                    ...
                    logger.warning(
                        f"Invalid status value '{filters['status']}' provided for SQLAlchemy filter. Skipping status filter."
                    )

            if "category" in filters and filters["category"]:
                logger.debug(
                    f"Applying SQLAlchemy filter: category == '{filters['category']}'"
                )
            ...
            logger.debug(
                f"Executing SQLAlchemy query: {query.statement.compile(compile_kwargs={'literal_binds': True})}"
            )  # Log compiled query (use with caution in prod)
            entities = query.all()
            ...
            if entities and logger.isEnabledFor(logging.DEBUG):
                ...
                logger.debug(
                    f"Sample SQLAlchemy entity (ID: {getattr(sample_entity, 'id', 'N/A')}): {log_dict}"
                )
            ...
            logger.debug(
                "Sensitive fields decrypted via SQLAlchemy path (if applicable)."
            )
            return decrypted_entities

        except Exception as e:
            logger.exception(
                f"Error in SQLAlchemy list method with filters {filters}: {e}"
            )
            raise DatabaseException(
        ...
        try:
            ...
            try:
                ...
                if use_sqlcipher:
                    ...
                    final_sql = " ".join(sql_parts)
                    logger.info(
                        f"Executing Supplier Count Query: {final_sql} with params: {params}"
                    )
                ...
                # --- SQLAlchemy Path ---
                    ...
                    count_query = select(func.count(self.model.id)).select_from(
                        self.model
                    )  # Count specific column like id

                    # Apply filters similar to _list_with_sqlalchemy
                    if "name" in filters and filters["name"]:
                        logger.debug(
                            f"Applying SQLAlchemy count filter: name LIKE '%{filters['name']}%'"
                        )
                    ...
                    if "status" in filters and filters["status"]:
                        try:
                            ...
                            if isinstance(status_enum, str):
                                status_enum = SupplierStatus(filters["status"].lower())
                            logger.debug(
                                f"Applying SQLAlchemy count filter: status == {status_enum}"
                            )
                            count_query = count_query.where(
                                self.model.status == status_enum
                            )
                        except ValueError:
                            logger.warning(
                                f"Invalid status value '{filters['status']}' provided for SQLAlchemy count filter. Skipping status filter."
                            )

                    if "category" in filters and filters["category"]:
                        logger.debug(
                            f"Applying SQLAlchemy count filter: category == '{filters['category']}'"
                        )
                        count_query = count_query.where(
                            self.model.category == filters["category"]
                        )

                    logger.debug(
                        f"Executing SQLAlchemy count query: {count_query.compile(compile_kwargs={'literal_binds': True})}"
                    )
                    result = self.session.execute(
                        count_query
                    ).scalar_one_or_none()  # Use scalar_one_or_none
                ...
                logger.warning(
                    "SQLCipher/SQLAlchemy import error in count, using fallback counting."
                )
            ...
            raise DatabaseException(
                message=f"Error counting suppliers: {str(e)}", entity_type="Supplier"
            )
    ...
    def stream_all(self, batch_size=50, **filters) -> Generator[Supplier, None, None]:
        ...
        logger.info(
            f"Streaming all suppliers with batch size {batch_size} and filters: {filters}"
        )
        offset = 0
        total_yielded = 0
        while True:
            ...
            if len(batch) < batch_size:
                logger.debug(
                    "Streaming finished: Last batch was smaller than batch size."
                )
                break
            if offset > 100000:  # Example safeguard limit
                logger.warning(
                    "Streaming stopped: Reached safeguard limit of 100,000 records."
                )
                break
    ...
    def search_suppliers(
        ...
        logger.info(
            f"Searching suppliers for '{query_str}'. Skip: {skip}, Limit: {limit}"
        )
        if not query_str:
            logger.warning(
                "Search called with empty query string. Returning empty list."
            )
            return []
        try:
            ...
            logger.debug(
                f"Executing SQLAlchemy search query: {search_query.statement.compile(compile_kwargs={'literal_binds': True})}"
            )
            entities = search_query.all()
            logger.info(
                f"Supplier search for '{query_str}' returned {len(entities)} results via ORM."
            )
            ...
            logger.error(
                f"Memory error during ORM supplier search for '{query_str}': {me}. Falling back to direct SQL."
            )
            ...
            raise DatabaseException(
                message=f"Error searching suppliers: {str(e)}", entity_type="Supplier"
            )

    def _search_suppliers_direct_sql(
        self, query_str: str, skip: int = 0, limit: int = 100
    ) -> List[Supplier]:
        """Fallback search implementation using direct SQL with logging."""
        logger.warning(
            f"Using direct SQL search fallback for query '{query_str}'. Skip: {skip}, Limit: {limit}"
        )
        sql = """
        ...
        try:
            logger.info(
                f"Executing Direct SQL Search Query: {sql} with params: {params}"
            )
            ...
            if use_sqlcipher:
                    ...
                    for i, column_name in enumerate(column_names):
                        ...
                        if hasattr(supplier, column_name):
                            ...
                            if column_name == "material_categories" and isinstance(
                                ...
                                try:
                                    setattr(supplier, column_name, json.loads(value))
                                except:
                                    setattr(supplier, column_name, [])
                            elif column_name == "status" and value is not None:
                                try:
                ...
                for row in result.mappings():  # Use .mappings() for dict-like rows
                    ...
                    for key, value in row.items():
                        if hasattr(supplier, key):
                            ...
                            if key == "material_categories" and isinstance(value, str):
                                try:
                                    setattr(supplier, key, json.loads(value))
                                except:
                                    setattr(supplier, key, [])
                            elif key == "status" and value is not None:
                                try:
                                    setattr(
                                        supplier, key, SupplierStatus(value.lower())
                                    )
                                except:
                    ...
                    entities.append(supplier)
                logger.info(
                    f"Direct SQL search via SQLAlchemy session returned {len(entities)} entities."
                )
            ...
            logger.exception(
                f"Error in direct SQL supplier search for '{query_str}': {e}"
            )
