...
from app.schemas.product import ProductFilter # Import the filter schema used by the API/Service
...
class ProductRepository(BaseRepository[Product]):
    ...
    def get_products_by_status(
        ...
        """
        logger.debug(f"Fetching products by status: {status.name}, skip: {skip}, limit: {limit}")
        query = (
    ...
    def get_products_by_pattern(
        ...
        """
        logger.debug(f"Fetching products by pattern ID: {pattern_id}, skip: {skip}, limit: {limit}")
    ...
    def get_products_by_price_range(
        ...
        """
        logger.debug(f"Fetching products by price range: {min_price}-{max_price}, skip: {skip}, limit: {limit}")
        query = self.session.query(self.model).filter(
            and_(
        ...
        entities = query.offset(skip).limit(limit).all()
        logger.debug(f"Found {len(entities)} products in price range {min_price}-{max_price}")
        return [self._decrypt_sensitive_fields(entity) for entity in entities]
    ...
    def update_product_pricing(
        ...
        if selling_price is not None:
            ...
            logger.debug(f"Setting selling_price to {selling_price} for product {product_id}")
    ...
    def search_products(
        ...
        """
        logger.debug(f"Searching products for query: '{query}', skip: {skip}, limit: {limit}")
        search_term = f"%{query}%"
    ...
    def update(self, id: int, data: Dict[str, Any]) -> Optional[Product]:
        ...
        """
        if 'quantity' in data:
            logger.warning(f"Attempted direct update of 'quantity' for product {id}. Ignoring. Use InventoryService.")
            del data['quantity']
        if 'status' in data:
            logger.warning(f"Attempted direct update of 'status' for product {id}. Ignoring. Use InventoryService.")
            del data['status']
        if 'storageLocation' in data:
            logger.warning(f"Attempted direct update of 'storageLocation' for product {id}. Ignoring. Use InventoryService.")
            del data['storageLocation']

        if not data: # If only invalid fields were passed
             logger.debug(f"No valid fields to update for product {id} after removing stock fields.")
    ...
    def get_by_id(self, id: int, load_inventory: bool = True) -> Optional[Product]:
        ...
        if entity:
             ...
             if load_inventory and hasattr(entity, 'inventory') and entity.inventory:
                 logger.debug(f" Eager loaded inventory record ID: {entity.inventory.id}")
             elif load_inventory:
                  logger.debug(f" Eager load requested, but no inventory record found or relationship not loaded for product {id}")
        else:
