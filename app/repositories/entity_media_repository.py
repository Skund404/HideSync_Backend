    ...
    def update_entity_media(
        ...
        try:
            ...
            if association:
                ...
                logger.info(f"Updating existing EntityMedia association ID: {association.id}")
            ...
            logger.error(f"Repository error in update_entity_media: {str(e)}", exc_info=True)
            raise # Re-raise for service layer to handle transaction

    def remove_entity_media(self, entity_type: str, entity_id: str, media_type: Optional[str] = None) -> int:
        """
        ...
        try:
            ...
            # Commit is handled by BaseRepository context or service layer transaction
            logger.info(f"Deleted {count} EntityMedia associations for {entity_type}:{entity_id} (media_type: {media_type})")
            return count
        except Exception as e:
            # Rollback should be handled by the service layer's transaction context
            logger.error(f"Repository error in remove_entity_media: {str(e)}", exc_info=True)
            raise # Re-raise for service layer to handle transaction
