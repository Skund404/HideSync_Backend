...
class EntityMediaService:
    ...
    def _attach_media_asset(self, entity_media_list: List[EntityMedia]) -> List[EntityMedia]:
    ...
    def get_by_entity(self, entity_type: str, entity_id: str) -> List[EntityMedia]:
        ...
        try:
            ...
            entity_media_list = self.entity_media_repository.get_by_entity(entity_type, prepared_id)
            return self._attach_media_asset(entity_media_list)
        except Exception as e:
    ...
    def get_thumbnail(self, entity_type: str, entity_id: str) -> Optional[EntityMedia]:
        ...
        try:
            ...
            thumbnail = self.entity_media_repository.get_thumbnail(entity_type, prepared_id)
    ...
    def _prepare_entity_id(self, entity_type: str, entity_id: Any) -> str:
        ...
        if entity_type == 'supplier':
            ...
            try:
                ...
                numeric_part = int(entity_id_val) if (entity_id_val := entity_id) is not None else None
                if numeric_part is not None:
                    ...
                    if formatted_id != entity_id_str:  # Log only if changed
                        logger.info(f"Formatted supplier entity_id from {entity_id_str} to {formatted_id}")
                    return formatted_id
        ...
        # Special handling for inventory IDs
            ...
            if (len(entity_id_str) == 36 and '-' in entity_id_str) or entity_id_str.startswith('inventory-'):
                return entity_id_str  # Assume it's already correct
            ...
            try:
                ...
                if numeric_part is not None:
                    ...
                    if formatted_id != entity_id_str:  # Log only if changed
                        logger.info(f"Formatted inventory entity_id from {entity_id_str} to {formatted_id}")
                    return formatted_id
            except (ValueError, TypeError):
                # If conversion fails, log a warning but proceed with the original string ID
                logger.warning(f"Could not format inventory entity_id '{entity_id_str}'. Using original value.")
    ...
    def _prepare_entity_media_data(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'entity_id' in prepared_data and 'entity_type' in prepared_data:
             prepared_data['entity_id'] = self._prepare_entity_id(
                 prepared_data['entity_type'],
                 prepared_data['entity_id']
             )
        ...
        prepared_data['caption'] = prepared_data.get('caption') # Allow None or empty string
    ...
    def create_entity_media(
        ...
        try:
            ...
            data_to_create['updated_at'] = data_to_create['created_at'] # Set updated_at on create
            ...
            if not created_association:
                 raise Exception("Failed to create entity media association using repository")

            logger.info(f"Successfully created entity media association {created_association.id}")
    ...
    def update_entity_media(
        ...
        try:
            ...
            logger.info(f"Attempting to update/create EntityMedia for {entity_type} {data_to_process['entity_id']} with media type {media_type}")
            ...
            if not result:
                 ...
                 raise Exception("Failed to create or update entity media via repository method")

            logger.info(f"Successfully updated/created entity media association {result.id}")
            ...
            if not final_result:
                ...
                logger.error(f"Could not retrieve entity media {result.id} after update/create.")
                raise Exception(f"Failed to retrieve entity media {result.id} post-operation")
            ...
            logger.error(f"Error updating/creating entity media: {str(e)}", exc_info=True)
    ...
    def remove_entity_media_by_asset_id(self, media_asset_id: str) -> bool:
        ...
        try:
            ...
            for record in entity_media_records:
                if self.delete_entity_media(record.id): # Use the service's delete method
                    count += 1
                else:
                     # Log if deletion fails for an individual record, but continue
                     logger.warning(f"Failed to delete entity media record {record.id} while removing by asset ID {media_asset_id}")

            logger.info(f"Attempted removal of {len(entity_media_records)} associations for asset {media_asset_id}. Successfully removed: {count}")
