...
class ProductCreated(DomainEvent):
    def __init__(
        ...
        name: str,
        sku: str,
        product_type: Optional[ProjectType],
        ...
        self.name = name
        self.sku = sku
        self.product_type = product_type.name if product_type else None
...
class ProductDeleted(DomainEvent):
    def __init__(
        self, product_id: int, name: str, sku: str, user_id: Optional[int] = None
    ):
        super().__init__()
        self.product_id = product_id
        self.name = name
...
class ProductService(BaseService[Product]):
    ...
    def __init__(
        ...
        if not inventory_service:
            logger.error(
                "CRITICAL: InventoryService was not provided to ProductService constructor!"
            )
    ...
    def create_product(
        self, product_in: ProductCreate, user_id: Optional[int] = None
    ) -> Product:
        """
        ...
        with self.transaction():  # Ensures atomicity
            ...
            product_data = product_in.model_dump(
                exclude={"quantity", "status", "storage_location", "sku"}
            )  # Exclude SKU initially too
            ...
            if product_in.sku:
                ...
                if not provided_sku:
                    raise ValidationException(
                        "Provided SKU cannot be empty.", {"sku": "Cannot be empty."}
                    )
                if self.repository.get_product_by_sku(provided_sku):
                    logger.warning(
                        f"SKU conflict during creation: '{provided_sku}' already exists."
                    )
                    raise BusinessRuleException(
                        f"SKU '{provided_sku}' already exists.", "PRODUCT_SKU_CONFLICT"
                    )
                ...
                for attempt in range(max_retries):
                    ...
                    generated_sku = self._generate_sku(
                        product_data["name"], product_data.get("product_type")
                    )
                    if not self.repository.get_product_by_sku(generated_sku):
                        final_sku = generated_sku
                        logger.info(
                            f"Generated unique SKU: {final_sku} (attempt {attempt + 1})"
                        )
                        break
                    logger.warning(
                        f"Generated SKU '{generated_sku}' collided (attempt {attempt + 1}). Retrying..."
                    )
                else:
                    logger.error(
                        f"Failed to generate a unique SKU after {max_retries} attempts."
                    )
                    raise HideSyncException(
                        "Failed to generate a unique SKU due to collisions."
                    )
            # --- End SKU Handling ---
            ...
            try:
                initial_quantity = (
                    product_in.quantity if product_in.quantity is not None else 0.0
                )
                ...
                logger.info(
                    f"Service: Inventory record created via InventoryService for product ID: {product.id}"
                )
            except Exception as e:
                logger.error(
                    f"Service: Failed to create inventory record for product {product.id}: {e}",
                    exc_info=True,
                )
                # The transaction context manager will handle rollback
                raise HideSyncException(
                    f"Failed to initialize inventory for product {product.id}"
                ) from e
            ...
            try:
                ...
                logger.error(
                    f"Failed to refresh product {product.id} to load inventory: {refresh_err}",
                    exc_info=True,
                )
                # Raise an exception as the returned object might be incomplete
                raise HideSyncException(
                    f"Failed to load inventory details for newly created product {product.id}"
                ) from refresh_err
            # --- END REFRESH ---
            ...
            logger.info(
                f"Service: Product {product.id} '{product.name}' created successfully."
            )
    ...
    def update_product(
        ...
        """
        logger.info(
            f"Service: Updating product ID: {product_id} by user ID {user_id or 'Unknown'}"
        )
        user_id = user_id or self._get_current_user_id()
        ...
        with self.transaction():
            ...
            product = self.repository.get_by_id(
                product_id, load_inventory=False
            )  # Don't need inventory loaded for this
            if not product:
                raise EntityNotFoundException("Product", product_id)

            # 2. Prepare update data
            update_data = product_update.model_dump(
                exclude_unset=True, exclude={"quantity", "status", "storage_location"}
            )

            if not update_data:
                logger.info(
                    f"Service: No valid fields provided to update for product {product_id}."
                )
                return product
            ...
            if new_sku and new_sku != product.sku:
                ...
                if existing and existing.id != product_id:
                    raise BusinessRuleException(
                        f"SKU '{new_sku}' already exists.", "PRODUCT_SKU_CONFLICT"
                    )
            ...
            if not updated_product:
                raise HideSyncException(
                    f"Update failed unexpectedly for product {product_id}"
                )
            ...
            if "reorder_point" in update_data and self.inventory_service:
                try:
                    ...
                    logger.info(
                        f"Reorder point changed for {product_id}, triggering inventory status re-evaluation."
                    )
                    # This requires InventoryService to have a method like this:
                    self.inventory_service.reevaluate_status(
                        item_type="product", item_id=product_id
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to trigger inventory status re-evaluation for product {product_id}: {e}",
                        exc_info=True,
                    )
    ...
    def delete_product(self, product_id: int, user_id: Optional[int] = None) -> bool:
        ...
        """
        logger.info(
            f"Service: Deleting product ID: {product_id} by user ID {user_id or 'Unknown'}"
        )
        user_id = user_id or self._get_current_user_id()
        ...
        with self.transaction():
            ...
            if self._has_active_sales(product_id):
                raise BusinessRuleException(
                    "Cannot delete product with active sales", "PRODUCT_ACTIVE_SALES"
                )

            # 3. Delete associated Inventory record via InventoryService
            try:
                deleted_inv = self.inventory_service.delete_inventory(
                    item_type="product", item_id=product_id
                )
                if not deleted_inv:
                    logger.warning(
                        f"Service: Inventory record for product {product_id} was not found or delete failed."
                    )
            except Exception as e:
                logger.error(
                    f"Service: Error deleting inventory for product {product_id}: {e}",
                    exc_info=True,
                )
                raise HideSyncException(
                    f"Failed to delete inventory for product {product_id}"
                ) from e
            ...
            if not deleted_prod:
                raise HideSyncException(
                    f"Product deletion failed unexpectedly for {product_id}"
                )
    ...
    def get_product_by_id(
        self, product_id: int, load_inventory: bool = True
    ) -> Optional[Product]:
        """Gets a product by ID, optionally loading inventory via relationship."""
        logger.debug(
            f"Service: Getting product by ID: {product_id}, load inventory: {load_inventory}"
        )
    ...
    def list_products_paginated(
        self, skip: int = 0, limit: int = 100, filters: Optional[ProductFilter] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        logger.info(
            f"Service: Listing products paginated: skip={skip}, limit={limit}, filters={filters.model_dump(exclude_none=True) if filters else 'None'}"
        )
        paginated_result = self.repository.list_products_paginated(
            skip=skip, limit=limit, filters=filters
        )
        logger.info(
            f"Service: Repository returned {len(paginated_result['items'])} items, total: {paginated_result['total']}"
        )
    ...
    def adjust_inventory(
        ...
        """
        logger.info(
            f"Service: Delegating inventory adjustment for product ID {product_id}: change={quantity_change}, reason='{reason}'"
        )
        user_id = user_id or self._get_current_user_id()
        ...
        if not product.inventory:
            raise HideSyncException(
                f"Inventory record missing for product {product_id}"
            )
        ...
        try:
            ...
            logger.info(
                f"Service: InventoryService successful adjustment for product ID {product_id}. New quantity: {new_quantity}"
            )

        except (
            Exception
        ) as e:  # Catch specific exceptions if needed (Insufficient, etc.)
            logger.error(
                f"Service: Inventory adjustment failed for product {product_id}: {e}",
                exc_info=True,
            )
    ...
    def get_low_stock_products(self) -> List[Dict[str, Any]]:
        ...
        low_stock_products: List[Product] = self.repository.get_products_low_in_stock(
            limit=1000
        )  # Use corrected repo method
        ...
        for product in low_stock_products:
            ...
            product_dict["percent_of_reorder"] = round(
    ...
    def get_all_product_margins(self) -> List[Optional[float]]:
        ...
        all_products = self.repository.list(
            limit=100000
        )  # Fetch all (potentially optimize)
    ...
    def _has_active_sales(self, product_id: int) -> bool:
        ...
        logger.warning(
            f"Placeholder check for active sales for product {product_id}. Returning False."
        )
        return False

    def _generate_sku(
        self, name: str, product_type: Optional[Union[ProjectType, str]] = None
    ) -> str:
        ...
        if product_type:
            type_str = (
                product_type.name
                if isinstance(product_type, ProjectType)
                else str(product_type)
            )
            type_mapping = {
                "WALLET": "WAL",
                "BAG": "BAG",
    ...
    class ProductService(BaseService[Product]):
        ...
        def calculate_cost_breakdown(self, product_id: int) -> Dict[str, Any]:
            ...
            """
            logger.info(
                f"Service: Calculating cost breakdown for product ID: {product_id}"
            )
            ...
            if not self.pattern_service or not self.material_service:
                ...
                raise HideSyncException(
                    "Required services for cost calculation are not configured."
                )
            ...
            product = self.repository.get_by_id(
                product_id, load_inventory=False
            )  # Don't need inventory loaded here
            if not product:
                raise EntityNotFoundException("Product", product_id)
            ...
            breakdown = {
                ...
                ),  # Get from product or default
                "overhead_costs": getattr(
                    product, "overhead_cost", 0.0
                ),  # Get from product or default
            ...
            if not pattern_id:
                logger.warning(
                    f"Product {product_id} has no associated pattern_id. Cannot calculate material costs."
                )
                ...
                try:
                    ...
                    if not hasattr(
                        ...
                        raise NotImplementedError(
                            "PatternService needs 'get_material_requirements_for_pattern'"
                        )
                    ...
                    logger.debug(
                        f"Material requirements for pattern {pattern_id}: {material_reqs}"
                    )

                    if not material_reqs:
                        logger.warning(
                            f"No material requirements found for pattern {pattern_id}."
                        )
                        breakdown["errors"].append(
                            f"No materials defined for pattern {pattern_id}."
                        )
                    else:
                        ...
                        for mat_id_str, req_data in material_reqs.items():
                            try:
                                ...
                                quantity_needed = float(
                                    req_data.get("quantity_required", 0.0)
                                )
                                unit_needed = req_data.get(
                                    "unit", "unknown_unit"
                                )  # Unit required by the pattern
                                ...
                                mat_cost_per_unit = getattr(
                                    material, "cost", 0.0
                                )  # Cost per material's base unit
                                mat_name = getattr(
                                    material, "name", f"Material {mat_id}"
                                )
                                # Safely get enum names or string values
                                mat_type_obj = getattr(
                                    material, "material_type", "UNKNOWN"
                                )
                                mat_type = (
                                    mat_type_obj.name
                                    if hasattr(mat_type_obj, "name")
                                    else str(mat_type_obj)
                                )
                                mat_unit_obj = getattr(material, "unit", "unknown_unit")
                                mat_unit = (
                                    mat_unit_obj.name
                                    if hasattr(mat_unit_obj, "name")
                                ...
                                breakdown["errors"].append(
                                    f"Invalid material ID format: {mat_id_str}"
                                )
                            except EntityNotFoundException as enf_mat:
                                # Catch if material_service.get_by_id raises this
                                logger.warning(
                                    f"Material referenced by pattern not found: {enf_mat}"
                                )
                                breakdown["errors"].append(
                                    f"Material ID {mat_id_str} not found."
                                )
                                ...
                                breakdown["errors"].append(
                                    f"Error processing material {mat_id_str}: {str(mat_err)[:100]}"
                                )  # Limit error message length

                except EntityNotFoundException as e:
                    # This catches if pattern_service.get_material_requirements... raises EntityNotFound for the pattern
                    logger.warning(
                        f"Pattern {pattern_id} not found for product {product_id} cost calculation: {e}"
                    )
                    breakdown["errors"].append(f"Pattern {pattern_id} not found.")
                except NotImplementedError as e:
                    logger.error(
                        f"Missing required service method for cost calculation: {e}"
                    )
                    ...
                    logger.error(
                        f"Error calculating material costs for product {product_id}: {e}",
                        exc_info=True,
                    )
                    breakdown["errors"].append(
                        f"Unexpected error during material cost calculation: {str(e)}"
                    )
                    # Decide if this should be a critical failure
            ...
            try:
                with self.transaction():  # Use the service's transaction context manager
                    ...
                    updated_product = self.repository.update(
                        product_id, cost_update_data
                    )
                    if not updated_product:
                        # This shouldn't happen if the product existed initially
                        logger.error(
                            f"Failed to update product {product_id} in DB after cost calculation."
                        )
                        breakdown["errors"].append(
                            "Failed to save calculated costs to product record."
                        )
                        # Raise an exception here if saving is critical
                        raise HideSyncException(
                            f"Failed to save calculated costs for product {product_id}"
                        )
                    else:
                        logger.info(
                ...
                logger.error(
                    f"Failed to save updated costs for product {product_id}: {update_err}",
                    exc_info=True,
                )
                breakdown["errors"].append(
                    f"Failed to save calculated costs to product record: {str(update_err)}"
                )
