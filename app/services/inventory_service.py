...
from app.schemas.inventory import (
    ...
    InventorySummaryResponse,  # Use specific schema for transaction input if defined
)
...
class InventoryAdjusted(DomainEvent):
    def __init__(
        ...
        self.adjustment_type = adjustment_type.name
        self.reason = reason
...
class InventoryService(BaseService[Inventory]):
    ...
    def reevaluate_status(self, item_type: str, item_id: int) -> Optional[Inventory]:
        ...
        logger.info(
            f"Service: Re-evaluating inventory status for {item_type} ID: {item_id}"
        )
        ...
        with self.transaction():
            ...
            if not inventory:
                logger.warning(
                    f"Inventory record not found for {item_type} {item_id} during status re-evaluation."
                )
                return None
            ...
            reorder_point = (
                item_details.get("reorder_point", 0.0) if item_details else 0.0
            )
            if not isinstance(
                reorder_point, (int, float)
            ):  # Handle non-numeric reorder points
                logger.warning(
            ...
            correct_status = self._determine_inventory_status(
            ...
            if inventory.status != correct_status:
                ...
                updated_inventory = self.repository.update(
                    inventory.id, {"status": correct_status}
                )
                if not updated_inventory:
                    logger.error(
                        f"Failed to update inventory status for {item_type} {item_id} in DB."
                    )
                    # Raise or handle error appropriately
                    raise HideSyncException(
                        f"Failed to save updated status for inventory {inventory.id}"
                    )
                ...
                if self.cache_service:
                    cache_key = self._get_cache_key(
                        item_type, item_id, suffix=":status"
                    )
    ...
    def product_service(self) -> "ProductService":
        ...
        if self._product_service_instance is None:
            ...
            )  # Get dependencies needed by ProductService
    ...
    def _get_current_user_id(self) -> Optional[int]:
        ...
        if (
            ...
            and self.security_context.current_user
        ):
    ...
    def create_inventory(
        ...
        """
        logger.info(
            f"Attempting to create inventory record for {item_type} ID: {item_id}"
        )
        ...
        if item_type not in ["product", "material", "tool"]:
            raise ValidationException(
                ...
                {"item_type": "Must be 'product', 'material', or 'tool'."},
            )
        ...
        with self.transaction():
            ...
            if existing:
                logger.warning(
                    f"Inventory record already exists for {item_type} ID {item_id}."
                )
                raise BusinessRuleException(
                    f"Inventory record already exists for {item_type} {item_id}.",
            ...
            logger.info(
                f"Inventory record created with ID: {inventory.id} for {item_type} ID {item_id}"
            )
    ...
    def delete_inventory(
        self, item_type: str, item_id: int, user_id: Optional[int] = None
    ) -> bool:
        """
        ...
        """
        logger.info(
            f"Attempting to delete inventory record for {item_type} ID: {item_id}"
        )
        user_id = user_id or self._get_current_user_id()
        item_type = item_type.lower()
        ...
        with self.transaction():
            ...
            if not inventory:
                logger.warning(
                    f"Inventory record not found for {item_type} ID {item_id} during deletion attempt."
                )
            ...
            if deleted:
                logger.info(
                    f"Inventory record ID {inventory_id} deleted for {item_type} ID {item_id}"
                )
                # Invalidate cache
                if self.cache_service:
                ...
                logger.error(
                    f"Deletion failed for inventory record ID {inventory_id} after finding it."
                )
                # Should not happen if get succeeded, raise an internal error?
                raise HideSyncException(
                    f"Inventory deletion failed unexpectedly for {item_type} {item_id}"
                )
    ...
    def adjust_inventory(
        ...
        reference_type: Optional[
            str
        ] = None,  # Type of reference ('sale', 'project', 'count')
        ...
        """
        logger.info(
            f"Adjusting inventory for {item_type} ID {item_id}: change={quantity_change}, type={adjustment_type.name}, reason='{reason}'"
        )
        user_id = user_id or self._get_current_user_id()
        item_type = item_type.lower()
        ...
        if not reason:
            raise ValidationException(
                "Reason is required for inventory adjustment.",
                {"reason": "Cannot be empty."},
            )
        if not isinstance(adjustment_type, InventoryAdjustmentType):
            raise ValidationException(
                "Invalid adjustment_type provided.",
                {"adjustment_type": "Must be InventoryAdjustmentType enum member."},
            )
        ...
        with self.transaction():
            ...
            if not inventory:
                ...
                if quantity_change > 0:
                    logger.warning(
                        f"Inventory record not found for {item_type} ID {item_id}. Creating one due to positive adjustment."
                    )
                    ...
                    logger.error(
                        f"Cannot apply negative adjustment: Inventory record not found for {item_type} ID {item_id}."
                    )
                    raise InsufficientInventoryException(
                        f"No inventory record found for {item_type} {item_id} to decrease quantity.",
                        "INVENTORY_NOT_FOUND_FOR_DECREASE",
                    )
            ...
            if not item_details and not created_inventory:
                ...
                logger.error(
                    f"Could not retrieve details for existing {item_type} ID {item_id} during adjustment."
                )
            ...
            new_status = self._determine_inventory_status(
                quantity=new_quantity, reorder_point=reorder_point
            )
            ...
            if to_location and inventory.storage_location != to_location:
                ...
                logger.info(
                    f"Updating storage location for {item_type} {item_id} to '{to_location}' during adjustment."
                )

            # 5. Update Inventory Record
            updated_inventory = self.repository.update(
                inventory.id, inventory_update_data
            )
            if not updated_inventory:  # Should not happen
                logger.error(
                    f"Failed to update inventory record ID {inventory.id} after validation."
                )
                raise HideSyncException(
                    f"Inventory update failed unexpectedly for {item_type} {item_id}"
                )
    ...
    def transfer_inventory(
        ...
        """
        logger.info(
            f"Attempting inventory transfer: {quantity} of {item_type} ID {item_id} from '{from_location}' to '{to_location}'"
        )
        user_id = user_id or self._get_current_user_id()
        item_type = item_type.lower()
        ...
        if quantity <= 0:
            raise ValidationException(
                "Transfer quantity must be positive.", {"quantity": "Must be > 0"}
            )
        ...
        if self.storage_service:
            if not self.storage_service.location_exists(from_location):
                logger.warning(
                    f"Source location '{from_location}' not found during transfer."
                )
                raise StorageLocationNotFoundException(from_location)
            if not self.storage_service.location_exists(to_location):
                logger.warning(
                    f"Destination location '{to_location}' not found during transfer."
                )
        ...
        with self.transaction():
            ...
            source_inventory = self.repository.get_inventory_by_item_and_location(
                item_type, item_id, from_location
            )
            if not source_inventory:
                logger.warning(
                    f"No inventory found for {item_type} ID {item_id} at source location '{from_location}'."
                )
                raise EntityNotFoundException(
                    f"Inventory for {item_type} {item_id} at location '{from_location}'"
                )
            if source_inventory.quantity < quantity:
                logger.warning(
                    f"Insufficient quantity for transfer: {item_type} {item_id} at '{from_location}'. Available: {source_inventory.quantity}, Requested: {quantity}"
                )
                raise InsufficientInventoryException(
            ...
            dest_inventory = self.repository.get_inventory_by_item_and_location(
                item_type, item_id, to_location
            )
            if not dest_inventory:
                logger.info(
                    f"No inventory record exists for {item_type} {item_id} at destination '{to_location}'. Creating one."
                )
                ...
                dest_status = self._determine_inventory_status(
                    quantity=quantity, reorder_point=reorder_point
                )
                ...
                logger.info(
                    f"Created destination inventory record ID {dest_inventory.id}"
                )
            else:
                ...
                item_details = self._get_item_details(
                    item_type, item_id
                )  # Need details for status
                reorder_point = (
                    item_details.get("reorder_point", 0.0) if item_details else 0.0
                )
                dest_new_status = self._determine_inventory_status(
                    quantity=dest_new_quantity, reorder_point=reorder_point
                )
                dest_inventory = self.repository.update(
                    dest_inventory.id,
                    {"quantity": dest_new_quantity, "status": dest_new_status},
                )
                logger.info(
                    f"Updated destination inventory record ID {dest_inventory.id} to quantity {dest_new_quantity}"
                )
            ...
            source_new_status = self._determine_inventory_status(
                quantity=source_new_quantity, reorder_point=reorder_point
            )

            # Decide whether to update or delete source inventory record if quantity becomes zero
            if source_new_quantity <= 0:
                logger.info(
                    f"Source inventory quantity reached zero for {item_type} {item_id} at '{from_location}'. Deleting record."
                )
                self.repository.delete(source_inventory.id)
            else:
                source_inventory = self.repository.update(
                    source_inventory.id,
                    {"quantity": source_new_quantity, "status": source_new_status},
                )
                logger.info(
                    f"Updated source inventory record ID {source_inventory.id} to quantity {source_new_quantity}"
                )
            ...
            self.transaction_repository.create_inventory_transaction(**transaction_data)
            logger.info(
                f"Transfer transaction logged for {item_type} ID {item_id} from '{from_location}' to '{to_location}'"
            )
    ...
    def get_summary_data(self) -> InventorySummaryResponse:
        ...
        try:
            ...
            out_of_stock_count = self.repository.count_by_status(
                InventoryStatus.OUT_OF_STOCK
            )
            ...
            )  # Assuming this returns {'total_value': ...}
            ...
            average_margin = (
                round(sum(valid_margins) / len(valid_margins), 1)
                if valid_margins
                else 0.0
            )
    ...
    def get_inventory_status(
        ...
        result = {
            ...
            "item_name": (
                item_details.get("name", f"Unknown {inventory.item_type}")
                if item_details
                else f"Unknown {inventory.item_type}"
            ),
            "quantity": inventory.quantity,
            "status": (
                inventory.status.name if inventory.status else None
            ...
            "last_updated": (
                inventory.updatedAt.isoformat()
                if hasattr(inventory, "updatedAt") and inventory.updatedAt
                else None
            ),
    ...
    def list_inventory_items(
        ...
        """
        logger.info(
            f"Listing inventory items: skip={skip}, limit={limit}, params={search_params.model_dump()}"
        )
    ...
    def get_inventory_transactions(
        ...
        """
        logger.info(
            f"Listing inventory transactions: skip={skip}, limit={limit}, item={item_type}:{item_id}, type={transaction_type}, date={start_date}-{end_date}"
        )
        # Convert string dates to datetime objects if provided
        dt_start = datetime.fromisoformat(start_date) if start_date else None
        dt_end = (
            datetime.fromisoformat(f"{end_date}T23:59:59.999999") if end_date else None
        )  # Include end day
    ...
    def get_low_stock_items(
        ...
        """
        logger.info(
            f"Fetching low stock items: threshold={threshold_percentage}%, type={item_type or 'All'}"
        )
        ...
        for (
            inventory,
            item_details,
        ) in low_stock_inventories:  # Assuming repo returns tuples
            if not item_details:  # Should ideally not happen if query is correct
                logger.warning(
                    f"Missing item details for low stock inventory ID {inventory.id} ({inventory.item_type} {inventory.item_id})"
                )
                continue
            ...
            results.append(
                {
                    ...
                    "item_type": inventory.item_type,
                    "item_id": inventory.item_id,
                    "item_name": item_details.get(
                        "name", f"Unknown {inventory.item_type}"
                    ),
                    "quantity": quantity,
                    "status": inventory.status.name if inventory.status else None,
                    "reorder_point": reorder_point,
                    "storage_location": inventory.storage_location,
                    "percent_of_reorder": round(
                        (quantity / reorder_point * 100) if reorder_point > 0 else 0, 1
                    ),
    ...
    def calculate_inventory_value(
        ...
        if not as_of_date:
            as_of_date = datetime.now(timezone.utc)  # Use timezone aware now
        ...
        try:
            ...
            logger.error(
                f"Failed to list inventory items for value calculation: {e}",
                exc_info=True,
            )
        ...
        for inventory in inventories:
            ...
            if item_details:
                ...
                if isinstance(cost_value, (int, float)):  # Check if it's a number
                    ...
                    logger.warning(
                        f"Invalid or missing cost for {inventory.item_type} ID {inventory.item_id}. Using 0.0 for value calculation."
                    )
            else:
                logger.warning(
                    f"Could not get item details for {inventory.item_type} ID {inventory.item_id}. Using 0.0 cost."
                )
            ...
            if type_key in by_type:
                ...
                if "other" not in by_type:
                    by_type["other"] = {"count": 0, "value": 0.0}
            ...
            items_detail.append(
                {
                    ...
                    "quantity": inventory.quantity,
                    "unit": (
                        item_details.get("unit", "unit") if item_details else "unit"
                    ),
                    "unit_cost": unit_cost,
                    "total_value": item_value,
        ...
        logger.info(
            f"Calculated total inventory value: {total_value:.2f} for {item_count} items."
        )
        return {
            "as_of_date": as_of_date.isoformat(),
    ...
    def generate_inventory_report(
        ...
        logger.info(
            f"Generating inventory report: type='{report_type}', filters={filters}"
        )
    ...
    def reconcile_inventory(
        ...
        logger.info(
            f"Reconciling inventory for {item_type} ID {item_id}: actual quantity={actual_quantity}"
        )
        user_id = user_id or self._get_current_user_id()
        inventory = self.repository.get_inventory_by_item_id(item_type.lower(), item_id)
        if not inventory:
            raise EntityNotFoundException(f"Inventory record for {item_type} {item_id}")
        ...
        if adjustment != 0:
            ...
            logger.info(
                f"No adjustment needed for {item_type} ID {item_id} (system quantity matches actual)."
            )
    ...
    def perform_inventory_audit(
        ...
        logger.info(
            f"Performing inventory audit prep: location={location_id}, type={item_type}"
        )
    ...
    def _get_item_details(
        ...
        try:
            if item_type == "product" and self.product_service:
                ...
                item = self.product_service.get_product_by_id(
                    item_id
                )  # Fetch the object
                if item:
                    details = {
                ...
                if item:
                    details = {
                        ...
                        "reorder_point": getattr(item, "reorder_point", 0.0),
                        "unit": (
                            getattr(item, "unit", "unit").name
                            if hasattr(getattr(item, "unit", None), "name")
                            else getattr(item, "unit", "unit")
                ...
                logger.warning(
                    f"No appropriate service configured or found for item_type: {item_type}"
                )
            ...
            logger.error(
                f"Error fetching details for {item_type} {item_id}: {e}", exc_info=True
            )
    ...
    def _determine_inventory_status(
        self, quantity: float, reorder_point: float
    ) -> InventoryStatus:
        """Determines inventory status based on quantity and reorder point."""
        # Ensure reorder_point is treated as a number
        numeric_reorder_point = (
            reorder_point if isinstance(reorder_point, (int, float)) else 0.0
        )
    ...
    def _check_low_stock(
        self, inventory: Inventory, reorder_point: Optional[float] = None
    ) -> None:
        ...
        if reorder_point > 0 and inventory.quantity <= reorder_point:
            ...
            logger.info(
                f"Low stock detected for {inventory.item_type} ID {inventory.item_id}. Qty: {inventory.quantity}, Reorder: {reorder_point}"
            )
            self.event_bus.publish(
                LowStockAlert(
                    inventory_id=inventory.id,
    ...
    def _generate_summary_report(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        inventories = self.repository.list_with_filters(
            skip=0,
            limit=10000,  # Fetch a large number for summary - adjust as needed
            status=search_params.status,
            location=search_params.location,
            item_type=search_params.item_type,
        ...
        summary = {
            ...
            "total_items_tracked": len(
                inventories
            ),  # Total distinct inventory records matching filters
        ...
        for inv in inventories:
            ...
            if item_type not in summary["summary_by_type"]:
                summary["summary_by_type"][item_type] = {
                    "count": 0,
                    "quantity": 0.0,
                    "value": 0.0,
                }
            ...
            if status_name not in summary["summary_by_status"]:
                summary["summary_by_status"][status_name] = {
                    "count": 0,
                    "quantity": 0.0,
                    "value": 0.0,
                }
            ...
            if location not in summary["summary_by_location"]:
                summary["summary_by_location"][location] = {
                    "count": 0,
                    "quantity": 0.0,
                    "value": 0.0,
                }
        ...
        # Round final values
        summary["total_quantity_on_hand"] = round(
            summary["total_quantity_on_hand"], 4
        ...
        for type_key in summary["summary_by_type"]:
            ...
            summary["summary_by_type"][type_key]["value"] = round(
                summary["summary_by_type"][type_key]["value"], 2
            )
        ...
        logger.info(
            f"Summary report generated successfully. Total items: {summary['total_items_tracked']}"
        )
        return summary
    ...
    def _generate_detail_report(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        total_count = (
            self.repository.count_with_filters(  # Assuming repo has this method
                status=search_params.status,
                location=search_params.location,
                item_type=search_params.item_type,
                search_term=search_params.search,
            ...
            report["items"].append(
                {
                    ...
                    "last_updated": (
                        inv.updatedAt.isoformat()
                        if hasattr(inv, "updatedAt") and inv.updatedAt
                        else None
                    ),
        ...
        logger.info(
            f"Detail report generated. Displaying {len(report['items'])} of {total_count} items."
        )
    ...
    def _generate_movement_report(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            end_date = datetime.fromisoformat(
                f"{end_date_str.split('T')[0]}T23:59:59.999999"
            )
        except ValueError:
            logger.warning(
                "Invalid date format in movement report filters. Using default range."
            )
        ...
        location_filter = filters.get(
            "location"
        )  # Filter by EITHER from_ or to_location matching this
            ...
            report["transactions"].append(
                {
                    ...
                    "item_id": tx.item_id,
                    "item_name": (
                        item_details.get("name", f"Unknown {tx.item_type}")
                        if item_details
                        else f"Unknown {tx.item_type}"
                    ),
                    "transaction_type": (
                        tx.transaction_type.name if tx.transaction_type else "UNKNOWN"
                    ),
                    "adjustment_type": (
                        tx.adjustment_type.name if tx.adjustment_type else None
                    ),
        ...
        report["summary"]["net_quantity_change"] = (
            report["summary"]["total_quantity_in"]
            - report["summary"]["total_quantity_out"]
        )

        # Round summary values
        report["summary"]["total_quantity_in"] = round(
            report["summary"]["total_quantity_in"], 4
        )
        report["summary"]["total_quantity_out"] = round(
            report["summary"]["total_quantity_out"], 4
        )
        report["summary"]["net_quantity_change"] = round(
            report["summary"]["net_quantity_change"], 4
        )
    ...
    def generate_inventory_report(
        ...
        if report_type_lower == "summary":
            ...
            )  # Default to at/below reorder point
            item_type = filters.get("item_type")
            ...
            raise NotImplementedError(
                f"Report type '{report_type}' is not implemented."
            )
