...
class KeyManager:
    ...
    def get_database_encryption_key() -> str:
        ...
        if settings.PRODUCTION:
            ...
            logger.info(f"Production mode detected. Using key management method: '{key_method}'")
             ...
             key_method = settings.KEY_MANAGEMENT_METHOD.lower()
             logger.warning(f"Non-production mode, no direct key. Using configured method: '{key_method}'")
        ...
        if key is None: # Only retrieve if not already set from settings
            if key_method == "file":
                ...
                key = KeyManager._get_key_from_gcp()
                 ...
                 logger.error("Non-production mode, but DATABASE_ENCRYPTION_KEY setting is empty.")
                 # Fall through to the check below
                 pass
            else:
                logger.error(f"Unsupported key management method configured: '{key_method}'")
        ...
        if not key:
            ...
            if settings.USE_SQLCIPHER:
                 raise SecurityException(f"{error_msg} Database key is mandatory (USE_SQLCIPHER=true).")
            else:
                 logger.warning(f"{error_msg} SQLCipher is disabled, proceeding without key.")
    ...
    @staticmethod
    def _get_key_from_file() -> Optional[str]: # Changed return type to Optional[str] for internal consistency
        """
        ...
        if not key_path_setting:
            logger.error("KEY_MANAGEMENT_METHOD is 'file' but KEY_FILE_PATH is not set in settings.")
            return None
        ...
        try:
            ...
            logger.info(f"Attempting to read key from file path: '{key_path}' (from setting: '{key_path_setting}')")
            ...
            try:
                ...
                if is_insecure:
                    logger.warning(f"Key file '{key_path}' has potentially unsafe permissions: {permissions_octal}")
                    if settings.ENFORCE_KEY_FILE_PERMISSIONS:
                        logger.error("Permission enforcement is ON. Raising SecurityException.")
                        raise SecurityException(f"Key file '{key_path}' has unsafe permissions ({permissions_octal})")
                    else:
                        logger.info("Permission enforcement is OFF. Proceeding despite unsafe permissions.")
                else:
                    logger.debug(f"Key file permissions appear secure ({permissions_octal}).")

            except OSError as perm_e:
                logger.error(f"Could not check permissions for key file '{key_path}': {perm_e}")
                # If we can't check permissions, assume failure if enforcement is on
                if settings.ENFORCE_KEY_FILE_PERMISSIONS:
                     logger.error("Permission enforcement is ON. Raising SecurityException due to check failure.")
                     raise SecurityException(f"Could not verify permissions for key file '{key_path}': {perm_e}")
                else:
                     logger.info("Permission enforcement is OFF. Proceeding despite permission check failure.")
            ...
            if not key:
                logger.error(f"Key file '{key_path}' was found but is empty or contains only whitespace.")
                return None
            ...
            if len(key) != expected_len:
                 logger.warning(f"Key read from '{key_path}' has unexpected length ({len(key)} chars). Expected {expected_len} hex chars.")
            ...
            logger.error(f"Key file not found at resolved path: '{key_path or key_path_setting}'")
            return None
        except IOError as io_e:
            logger.error(f"IOError reading key file '{key_path or key_path_setting}': {io_e}")
            return None
        except OSError as os_e:
             logger.error(f"OSError accessing key file '{key_path or key_path_setting}': {os_e}")
             return None
            ...
            logger.exception(f"Unexpected error reading key file '{key_path or key_path_setting}': {e}")
            return None
    ...
    def _get_key_from_environment() -> Optional[str]: # Changed return type
        ...
        if not env_var:
            logger.error("KEY_MANAGEMENT_METHOD is 'environment' but KEY_ENVIRONMENT_VARIABLE is not set.")
    ...
    def _get_key_from_aws() -> str:
        ...
        try:
            ...
            if not secret_name or not region:
                 raise SecurityException("AWS_SECRET_NAME and AWS_REGION must be set for AWS method.")
            ...
            if "SecretString" in response:
                ...
                try:
                    ...
                    if key_field and key_field in secret_dict:
                        ...
                        logger.info(f"Successfully extracted key from field '{key_field}' in AWS secret.")
                        return key
                    else:
                        logger.info("AWS secret is JSON, but key field not specified or found. Returning entire JSON string.")
                        return secret # Return whole JSON string if field not specified/found
                except json.JSONDecodeError:
                    logger.info("AWS secret is not JSON. Returning secret string directly.")
                ...
                return key
                 ...
                 logger.error(f"No SecretString or SecretBinary found in AWS response for '{secret_name}'")
                 raise SecurityException(f"Invalid response from AWS Secrets Manager for secret '{secret_name}'")

        except ImportError:
            logger.error("boto3 library not installed, required for AWS Secrets Manager")
            raise SecurityException("boto3 library not installed, required for AWS Secrets Manager")
        except Exception as e:
            logger.exception(f"Error retrieving key from AWS Secrets Manager: {e}") # Use logger.exception for traceback
    ...
    def _get_key_from_azure() -> str:
        ...
        try:
            ...
            if not vault_url or not secret_name:
                 raise SecurityException("AZURE_VAULT_URL and AZURE_SECRET_NAME must be set for Azure method.")
            ...
            if not retrieved_secret or not retrieved_secret.value:
                 logger.error(f"Secret '{secret_name}' retrieved from Azure Key Vault but value is empty.")
                 raise SecurityException(f"Secret '{secret_name}' retrieved from Azure Key Vault but value is empty.")

            logger.info(f"Successfully retrieved secret '{secret_name}' from Azure Key Vault.")
            return retrieved_secret.value

        except ImportError:
            logger.error("azure-keyvault-secrets and azure-identity libraries not installed, required for Azure Key Vault")
            raise SecurityException("azure libraries not installed, required for Azure Key Vault")
        except Exception as e:
    ...
    def _get_key_from_gcp() -> str:
        ...
        try:
            ...
            version_id = settings.GCP_SECRET_VERSION or "latest" # Default to latest if not specified
            if not project_id or not secret_id:
                 raise SecurityException("GCP_PROJECT_ID and GCP_SECRET_ID must be set for GCP method.")

            client = secretmanager.SecretManagerServiceClient()
            name = f"projects/{project_id}/secrets/{secret_id}/versions/{version_id}"

            logger.debug(f"Requesting secret version '{name}' from Google Secret Manager")
            response = client.access_secret_version(request={"name": name})

            if not response or not response.payload or not response.payload.data:
                 logger.error(f"Invalid or empty payload received from Google Secret Manager for '{name}'")
                 raise SecurityException(f"Invalid or empty payload received from Google Secret Manager for '{name}'")

            key = response.payload.data.decode("UTF-8")
            logger.info(f"Successfully retrieved secret '{secret_id}' version '{version_id}' from Google Secret Manager.")
            return key

        except ImportError:
            logger.error("google-cloud-secret-manager library not installed, required for GCP Secret Manager")
            raise SecurityException("google-cloud-secret-manager library not installed, required for GCP Secret Manager")
        except Exception as e:
    ...
    def create_key_file(key: Optional[str] = None) -> str:
        ...
        if not key_file_setting:
             raise SecurityException("Cannot create key file: KEY_FILE_PATH is not set in settings.")
        ...
        try:
            ...
            if not os.path.isdir(key_dir):
                 logger.info(f"Creating key directory: '{key_dir}' with secure permissions (0o700)")
